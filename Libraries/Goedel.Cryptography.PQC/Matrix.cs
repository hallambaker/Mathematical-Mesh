...
public struct Matrix {
    ...
    public static Matrix GenerateMatrix(int kyberK, byte[] seed, bool transposed = false) {
        var a = new Matrix(kyberK);
        ulong[] state;
        ...
        for (var i = 0; i < kyberK; i++) {
            for (var j = 0; j < kyberK; j++) {
                ...
                ctr = a.Poly[i].Coefficients[j].RejUniform(buf, buflen);
                ...
                while (ctr < Kyber.N) {
                    ...
                    ctr = a.Poly[i].Coefficients[j].RejUniform(buf, buflen, ctr);
                    }
                }
            }
    ...
    public string GetHash() {
        ...
        for (var i = 0; i < d0; i++) {
            for (var j = 0; j < d1; j++) {
                for (var k = 0; k < d2; k++) {
                    buffer[offset++] = (byte)(Poly[i].Coefficients[j].Values[k] & 0xff);
                    buffer[offset++] = (byte)(Poly[i].Coefficients[j].Values[k] >> 8);
                    }
                }
            }
