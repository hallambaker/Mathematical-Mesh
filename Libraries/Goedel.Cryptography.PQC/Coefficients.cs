...
public struct Coefficients {
    ...
    public static Coefficients CBD2(byte[] input) {
        (input.Length == (2 * Kyber.N) / 4).AssertTrue(NYI.Throw);
        var result = new Coefficients();
    ...
    /// <param name="input">input byte array</param>
    /// <returns>output polynomial</returns>
    public static Coefficients CBD3(byte[] input) {
        ...
        for (uint len = 128; len >= 2; len >>= 1) {
            for (uint start = 0; start < 256; start = j + len) {
                ...
                for (j = start; j < start + len; ++j) {
                    t = Fqmul(zeta, Values[j + len]);
                    Values[j + len] = (short) (Values[j] - t);
                    Values[j] = (short)(Values[j] + t);
                    }
                }
            }
        }
    ...
    private void InvNTT() {
        ...
        for (uint len = 2; len <= 128; len <<= 1) {
            for (uint start = 0; start < 256; start = j + len) {
                ...
                for (j = start; j < start + len; ++j) {
                    t = Values[j];
                    Values[j] = Kyber.BarrettReduce((short)(t + Values[j + len]));
                    Values[j + len] = (short)(t - Values[j + len]);
                    Values[j + len] = Fqmul(zeta, Values[j + len]);
                    }
                }
            }

        for (j = 0; j < 256; ++j)
            Values[j] = Fqmul(Values[j], zetas_inv[127]);
        }
    ...
    public void Reduce() {
        for (var i = 0; i < Kyber.N; i++) {
            Values[i] = Kyber.BarrettReduce(Values[i]);
            }
        }
    ...
    public void PolyToMont() {
        ...
        for (var i = 0; i < Kyber.N; i++) {
            Values[i] = Kyber.MontgomeryReduce((short)(Values[i]*f));
            }
        }
    ...
    public void PolyCSubQ() {
        for (var i = 0; i < Kyber.N; i++) {
            Values[i] = Kyber.ConditionalSubtract(Values[i]);
            }
        }
    ...
    public string GetHash() {
        ...
        var offset = 0;
        for (var k = 0; k < d2; k++) {
            buffer[offset++] = (byte)(Values[k] & 0xff);
            buffer[offset++] = (byte)(Values[k] >> 8);
            }
